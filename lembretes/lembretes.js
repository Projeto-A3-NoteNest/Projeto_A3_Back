...
app.post("/lembretes", async (req, res) => {
  const { nome_lembrete, data_lembrete, categoria } = req.body;
  console.log("-----------------------");
  ...
  const [results] = await db.execute(
    "INSERT INTO lembretes (nome_lembrete, data_lembrete, categoria) VALUES (?, ?, ?)",
    [nome_lembrete || null, data_lembrete || null, categoria || null]
  );
...
app.put("/lembretes/:id_lembrete", async (req, res) => {
  ...
  if (lembrete.length > 0) {
    await db.execute(
      ...
      [req.body.nome_lembrete, req.body.data_lembrete, idLembrete]
    );
